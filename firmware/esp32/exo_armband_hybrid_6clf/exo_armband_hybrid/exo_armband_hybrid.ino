...
static const char* CHARACTERISTIC_UUID_WEIGHTS = "abcd1234-5678-1234-5678-abcdef123458";  // model weights (write) + ack (notify)
...
BLECharacteristic* pCharacteristicWeights;  // model weights receive + ack
bool deviceConnected = false;

// ─────────────────────────────────────────────────────────────────────
// BLE weights-receive protocol
//   [4B magic 0xDEADBEEF][4B payload length][payload][4B CRC32 of payload]
//   payload = W0 b0 W1 b1 ... means stds (float32), matches MODEL_TOPOLOGY.
// Written by lib/ble_weights.py on the PC in 244-byte write-with-response
// chunks. onWrite() below must never block (no delay()/ESP.restart()) —
// the ATT Write Response for the very last chunk can't go out while the
// callback is still running, so a pending restart is scheduled and
// actually performed from loop() instead.
// ─────────────────────────────────────────────────────────────────────
#define WEIGHTS_MAGIC 0xDEADBEEFUL
#define WEIGHTS_HEADER_SIZE 8   // magic(4) + length(4)
#define WEIGHTS_CRC_SIZE    4
#define WEIGHTS_PAYLOAD_SIZE \
  ((size_t)(NN_TOTAL_WEIGHTS + NN_TOTAL_BIASES + 2 * N_FEATURES) * sizeof(float))
#define WEIGHTS_FRAME_SIZE (WEIGHTS_HEADER_SIZE + WEIGHTS_PAYLOAD_SIZE + WEIGHTS_CRC_SIZE)

static uint8_t weightsRecvBuf[WEIGHTS_FRAME_SIZE];
static size_t  weightsRecvOffset = 0;
...
static void notifyWeightsResult(const char* msg) {
  if (deviceConnected) {
...
class WeightsCharCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) {
    ...
    if (weightsRecvOffset + len > WEIGHTS_FRAME_SIZE) {
      Serial.println("[weights] overflow, resetting transfer");
      weightsRecvOffset = 0;
      notifyWeightsResult("ERR:OVERFLOW");
      return;
    }

    memcpy(weightsRecvBuf + weightsRecvOffset, pChar->getData(), len);
    weightsRecvOffset += len;

    if (weightsRecvOffset < WEIGHTS_FRAME_SIZE) {
      return;  // wait for more chunks
    }

    // Full frame received. Reset the offset for the next transfer up front
    // so a bad frame doesn't wedge future attempts.
    weightsRecvOffset = 0;

    uint32_t magic = 0, declaredLen = 0;
    memcpy(&magic, weightsRecvBuf, 4);
    memcpy(&declaredLen, weightsRecvBuf + 4, 4);

    if (magic != WEIGHTS_MAGIC || declaredLen != WEIGHTS_PAYLOAD_SIZE) {
      Serial.println("[weights] bad header");
      notifyWeightsResult("ERR:HEADER");
      return;
    }

    const uint8_t* payload = weightsRecvBuf + WEIGHTS_HEADER_SIZE;
    uint32_t crcReceived = 0;
    memcpy(&crcReceived, weightsRecvBuf + WEIGHTS_HEADER_SIZE + WEIGHTS_PAYLOAD_SIZE, 4);
    uint32_t crcComputed = computeCRC32(payload, WEIGHTS_PAYLOAD_SIZE);

    if (crcComputed != crcReceived) {
      Serial.println("[weights] CRC mismatch");
      notifyWeightsResult("ERR:CRC");
      return;
    }

    File f = LittleFS.open("/weights.tmp", "w");
    if (!f) {
      Serial.println("[weights] cannot open /weights.tmp");
      notifyWeightsResult("ERR:OPEN");
      return;
    }
    size_t written = f.write(payload, WEIGHTS_PAYLOAD_SIZE);
    f.close();
    if (written != WEIGHTS_PAYLOAD_SIZE) {
      LittleFS.remove("/weights.tmp");
      Serial.println("[weights] short write to /weights.tmp");
      notifyWeightsResult("ERR:WRITE");
      return;
    }

    // Atomic swap: only replace the working model once the new one is
    // fully and correctly on disk. A failed transfer never corrupts the
    // model the bracelet is currently running.
    LittleFS.remove("/weights.bin");  // fine if it doesn't exist yet
    if (!LittleFS.rename("/weights.tmp", "/weights.bin")) {
      Serial.println("[weights] rename failed");
      notifyWeightsResult("ERR:RENAME");
      return;
    }

    Serial.println("[weights] received OK, restarting to load it");
    notifyWeightsResult("OK:WEIGHTS");

    // Schedule the restart instead of doing it here — see comment above
    // WeightsCharCallbacks.
    pendingRestart = true;
...
void setup() {
  ...
  // Third characteristic: receive model weights (write) + ack (notify)
  ...
  // NEW: load the model from LittleFS (/weights.bin), sent over BLE by the
  // PC app's "Send weights over BLE" action. If no model has been sent yet
  // (fresh flash, or LittleFS had to be formatted), isLoaded() stays false
  // and runInference() below is skipped — raw EMG + BLE still work fine.
  if (!LittleFS.begin(true)) {
    Serial.println("[littlefs] mount/format failed");
  } else if (nn.loadFromLittleFS("/weights.bin", preproc)) {
    Serial.println("# model loaded from /weights.bin");
  } else {
    Serial.println("# no model loaded yet - waiting for weights over BLE");
  }
}
...
// =========================
void loop() {
  // NEW: perform a restart requested by WeightsCharCallbacks::onWrite(),
  // deferred here because the callback itself must return quickly so the
  // BLE stack can still send the ATT Write Response for the last chunk.
  ...
  // Gated on nn.isLoaded(): before any weights have been received over
  // BLE (or right after a fresh flash), skip inference entirely rather
  // than run on all-zero weights. Raw EMG streaming is unaffected.
  if (nn.isLoaded() &&
